...
import static com.qkopy.textviewformatter.QkoTextViewFormatterUtil.*;

public class QkoViewCompat {


    /**
     * QkoViewCompat for EditText.
    ...
    private static void format(EditText editText, TextWatcher mainWatcher, TextWatcher[] otherWatchers ) {
        ...
        CharSequence formatted = QkoViewCompat.extractFlagsForEditText(text);
    ...
    private static void sendAfterTextChanged(TextWatcher[] mListeners, Editable s) {
        if (mListeners != null) {
            for (TextWatcher mListener : mListeners) {
    ...
    public static CharSequence extractFlagsForTextView(CharSequence text) {
        ...
        ArrayList<QkoTextViewFormatterUtil.Flag> flags = new ArrayList<>();

        QkoTextViewFormatterUtil.Flag boldFlag = new QkoTextViewFormatterUtil.Flag(INVALID_INDEX, INVALID_INDEX, BOLD_FLAG);
        QkoTextViewFormatterUtil.Flag strikeFlag = new QkoTextViewFormatterUtil.Flag(INVALID_INDEX, INVALID_INDEX, STRIKE_FLAG);
        QkoTextViewFormatterUtil.Flag italicFlag = new QkoTextViewFormatterUtil.Flag(INVALID_INDEX, INVALID_INDEX, ITALIC_FLAG);
        ...
        for (int i = 0, j = 0; i < textChars.length; i++) {
            ...
            if (c == BOLD_FLAG) {
                if (boldFlag.start == INVALID_INDEX) {
                    if (QkoTextViewFormatterUtil.hasFlagSameLine(text, BOLD_FLAG, i + 1)) {
                        boldFlag.start = j;
                        continue;
                    }
                } else {
                    boldFlag.end = j;
                    flags.add(boldFlag);
                    boldFlag = new QkoTextViewFormatterUtil.Flag(INVALID_INDEX, INVALID_INDEX, BOLD_FLAG);
                    continue;
                }
            }
            if (c == STRIKE_FLAG) {
                if (strikeFlag.start == INVALID_INDEX) {
                    if (QkoTextViewFormatterUtil.hasFlagSameLine(text, STRIKE_FLAG, i + 1)) {
                        strikeFlag.start = j;
                        continue;
                    }
                } else {
                    ...
                    strikeFlag = new QkoTextViewFormatterUtil.Flag(INVALID_INDEX, INVALID_INDEX, STRIKE_FLAG);
                    continue;
                }
            }
            if (c == ITALIC_FLAG) {
                if (italicFlag.start == INVALID_INDEX) {
                    if (QkoTextViewFormatterUtil.hasFlagSameLine(text, ITALIC_FLAG, i + 1)) {
                        italicFlag.start = j;
                        continue;
                    }
                } else {
                    ...
                    italicFlag = new QkoTextViewFormatterUtil.Flag(INVALID_INDEX, INVALID_INDEX, ITALIC_FLAG);
                    continue;
                }
            }
            characters.add(c);
        ...
        String formatted = QkoTextViewFormatterUtil.getText(characters);
        SpannableStringBuilder builder = new SpannableStringBuilder(formatted);


        for (int i = 0; i < flags.size(); i++) {

            QkoTextViewFormatterUtil.Flag flag = flags.get(i);
